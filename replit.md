...
- **ETL Data Warehouse Tables**: Database schema for ETL processes in the PostgreSQL schema "ventas" with 8 staging tables (stg_maeedo, stg_maeddo, stg_maeen, stg_maepr, stg_maeven, stg_tabbo, stg_tabpp) for importing data from SQL Server and a fact_ventas table with 81 columns (79 original + ESDO + ESPGDO) for the final denormalized data. Tables include optimized indexes for query performance (ix_fact_ventas_feemli_id, ix_fact_ventas_cliente, ix_fact_ventas_producto, ix_fact_ventas_vendedor, ix_fact_ventas_bodega). ETL execution log (watermark) table tracks all ETL runs with execution status, period, document types, branches processed, records count, execution time, detailed statistics in JSON format, and watermark timestamp for last processed date. Functional ETL process (server/etl-process.ts) extracts from SQL Server (PANORAMICA database), loads to staging tables, processes into fact_ventas with calculated monto column (CAPRCO2 * PPPRNE for regular docs, CAPRCO2 * PPPRNE * -1 for NCV credit notes), supports multiple branches (004, 006, 007), processes document types (FCV, GDV, FVL, NCV, BLV, FDV), includes descriptive names (nokoen, nokofudo, nokoprct, nosudo) via LEFT JOINs to MAEEN, TABFU, TABSU, TABBO tables, handles empty/null numeric values with cleanNumeric helper (filters non-numeric characters), uses batch inserts (50 records per batch) to avoid parameter limits, supports annual execution mode (processes all months of a year sequentially), filters GDV (Guías de Venta) to only load pending documents (ESDO IS NULL OR ESDO = '') matching Excel export behavior, and automatically logs all executions for audit and monitoring. GDV filtering ensures only open/pending sales guides are included, excluding closed/processed ones. 100% schema compatibility verified against ERP CSV exports with all 81 columns matched. Production deployment files available: production-etl-setup.sql (schema creation script) and INSTRUCCIONES-ETL-PRODUCCION.md (deployment guide).
