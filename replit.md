...
- **Internal Notifications System**: Comprehensive notification management with role-based creation and archiving. Features include: Three notification types (personal, general, departamento), priority levels (baja, media, alta, crítica), department targeting (Logística, Laboratorio, Finanzas, Ventas, Producción, Planificación), read tracking per user with notification_reads table, notification bell in sidebar with unread counter, dedicated notifications page with tabs (Activas, Archivadas, Crear Nueva), filtering by type/priority/department, and read statistics showing who has read each notification. Security: Only authorized roles (admin, supervisor, logistica_bodega, logistica, laboratorio, area_produccion, area_logistica, area_aplicacion, produccion, planificacion) can create notifications; only admin and supervisor can archive. Notifications cannot be deleted, only archived, ensuring full traceability.
