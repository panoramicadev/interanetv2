...
- **Inventory Module (Hybrid System)**: Real-time stock levels with integrated average pricing, using a hybrid architecture that combines PostgreSQL product catalog with live SQL Server stock queries. **Product Catalog**: PostgreSQL `inventory_products` table stores synchronized product master data (sku, nombre, unidad1, unidad2, categoria, precioMedio, activo, ultimaSincronizacion, sincronizadoPor) synced from SQL Server MAEPR table via admin/supervisor-triggered "Sincronizar Catálogo" button. **Sync Process**: Secure synchronization with Zod validation, mutex lock to prevent concurrent syncs, 10-minute timeout protection, automatic error handling, and comprehensive audit trail in `inventory_sync_log`. Validation errors are logged (max 50 shown) and sync status set to 'partial' if errors exist. **Hybrid Queries**: System JOINs PostgreSQL catalog with real-time SQL Server MAEST stock data, providing product info from catalog + live stock quantities from ERP. Automatic fallback to direct SQL Server query when catalog is empty. **Features**: Stock quantities by warehouse/branch, average price (PM - Precio Medio) display, total inventory value calculation (stock × average price), warehouse/branch filtering, low stock alerts, comprehensive summary metrics, and manual catalog sync with last sync timestamp display. **Data Integrity**: Spanish field names throughout (nombre, unidad1, unidad2, categoria, precioMedio, activo, ultimaSincronizacion, sincronizadoPor), correct SQL-to-PostgreSQL field mappings (KOPR→sku, NOKOPR→nombre, PM→precioMedio), and transaction-based updates for atomicity. All numeric values use Chilean formatting (es-CL) with safe undefined handling.
...
- **Internal Notifications System**: Comprehensive notification management with role-based creation, archiving, and automatic event notifications. Features include: Three notification types (personal, general, departamento), priority levels (baja, media, alta, crítica), department targeting (Logística, Laboratorio, Finanzas, Ventas, Producción, Planificación), read tracking per user with notification_reads table, "Notificaciones" menu item at top of sidebar with special golden border styling, dedicated notifications page (/notificaciones) with tabs (Activas, Archivadas, Crear Nueva), filtering by type/priority/department, and read statistics showing who has read each notification. Security: Only authorized roles (admin, supervisor, logistica_bodega, logistica, laboratorio, area_produccion, area_logistica, area_aplicacion, produccion, planificacion) can create notifications; only admin and supervisor can archive. Notifications cannot be deleted, only archived, ensuring full traceability. **Automatic Notifications**: System automatically generates notifications for key events including: new reclamos created, mantención solicitudes, CRM leads, gastos empresariales, marketing solicitudes, visitas técnicas, and inactive clients alerts (daily check). Daily scheduler checks for inactive clients (no purchases >30 days, <365 days) and sends high-priority notification to Sales department when detected. All automatic notifications include relevant context and action URLs for quick navigation. Notifications use intelligent department targeting to notify the right teams.
