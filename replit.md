...
- **Internal Notifications System**: Comprehensive notification management with role-based creation and archiving. Features include: Three notification types (personal, general, departamento), priority levels (baja, media, alta, crítica), department targeting (Logística, Laboratorio, Finanzas, Ventas, Producción, Planificación), read tracking per user with notification_reads table, floating notification bell in top-right corner (circular, gradient blue design with unread badge), notification panel integrated in sidebar showing latest 3 notifications with quick preview, dedicated notifications page (/notificaciones) with tabs (Activas, Archivadas, Crear Nueva), filtering by type/priority/department, and read statistics showing who has read each notification. Security: Only authorized roles (admin, supervisor, logistica_bodega, logistica, laboratorio, area_produccion, area_logistica, area_aplicacion, produccion, planificacion) can create notifications; only admin and supervisor can archive. Notifications cannot be deleted, only archived, ensuring full traceability. UI design: Floating bell always visible in top-right corner, sidebar panel shows compact notification preview, both components auto-refresh every 30 seconds.
