...
- **Health Monitor Fix**: Updated health check endpoint in `server/routes.ts` to use `start_time` instead of deprecated `execution_date` field when querying ventas.etl_execution_log, preventing "column does not exist" errors in production.
- **NVV Schema Fix (Migration 008)**: Added missing `ruen` column to `nvv.stg_maeen_nvv` table for client segment mapping. This column is required by the NVV ETL process to map client segments correctly. Migration includes idempotent check to prevent errors on existing installations.