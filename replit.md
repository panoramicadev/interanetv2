...
- **Inventory Module**: Real-time stock levels with integrated average pricing from SQL Server ERP. Features include: stock quantities by warehouse, average price (PM - Precio Medio) display, total inventory value calculation (stock × average price), warehouse filtering, low stock alerts, and comprehensive summary metrics. Data is fetched directly from SQL Server MAEPR table (Maestro de Productos) with STFI1/STFI2 stock fields and PM price field. Summary dashboard displays: Total Products, Stock Total, Available Units, **Valor Total Inventario** (total inventory valuation in CLP format), and Low Stock alerts. Inventory table shows: SKU, Product Name, Warehouse, Stock Total, Unit, **Precio Medio** (average price in CLP), **Valor Inventario** (line total value in CLP), and Status badges. All numeric values use Chilean formatting (es-CL) with safe handling of undefined values during data loading.
