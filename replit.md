...
- **Fact Ventas (Sales Data Warehouse)**: Advanced sales analytics module with staging tables and fact table architecture imported from SQL Server. Features include 8 staging tables (stg_maeedo, stg_maeddo, stg_maeen, stg_maepr, stg_maeven, stg_tabbo, stg_tabpp) for data normalization and a comprehensive fact_ventas table with 79 columns for detailed sales analysis. Provides advanced filtering by date range, client, product, and salesperson, real-time statistics dashboard showing total sales, units sold, and transaction count, optimized database indexes for fast query performance, and detailed transaction history view. Accessible to admin, supervisor, and salesperson roles via sidebar navigation.
