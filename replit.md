...
- **ORM**: Drizzle ORM
- **Schema**: Users, sessions, sales_transactions, ecommerceOrders, ecommerceProductGroups, notifications, tasks, task_assignments, reclamosGenerales, reclamosGeneralesPhotos, reclamosGeneralesHistorial, presupuestoMarketing, solicitudesMarketing
...
- **ETL System (Ventas)**: Automated Extract-Transform-Load system that synchronizes sales data from SQL Server (Panoramica ERP) to PostgreSQL for analytics. Features include incremental watermark-based extraction (prevents duplicate processing), staging tables for data normalization, fact_ventas analytical table with pre-calculated metrics, configurable filters (2025+, document types: FCV/FVL/NCV, excludes vendor FHP), web-based ETL execution interface with real-time statistics, connection pooling for SQL Server, and admin/supervisor role access. Schema includes ventas.etl_estado (watermark tracking), staging tables (stg_maeedo, stg_maeddo, stg_maeen, stg_maepr, stg_maeven, stg_tabbo, stg_tabpp), and ventas.fact_ventas (denormalized analytical table with indexed columns for performance).
