...
- **Internal Notifications System**: Comprehensive notification management with role-based creation, archiving, and automatic event notifications. Features include: Three notification types (personal, general, departamento), priority levels (baja, media, alta, crítica), department targeting (Logística, Laboratorio, Finanzas, Ventas, Producción, Planificación), read tracking per user with notification_reads table, floating notification bell in top-right corner (circular, gradient blue/gold design with unread badge), "Notificaciones" menu item at top of sidebar with special golden border styling, dedicated notifications page (/notificaciones) with tabs (Activas, Archivadas, Crear Nueva), filtering by type/priority/department, and read statistics showing who has read each notification. Security: Only authorized roles (admin, supervisor, logistica_bodega, logistica, laboratorio, area_produccion, area_logistica, area_aplicacion, produccion, planificacion) can create notifications; only admin and supervisor can archive. Notifications cannot be deleted, only archived, ensuring full traceability. **Automatic Notifications**: System automatically generates notifications for key events including: new reclamos created, mantención solicitudes, CRM leads, gastos empresariales, marketing solicitudes, and visitas técnicas. All automatic notifications include relevant context and action URLs for quick navigation. Notifications use intelligent department targeting to notify the right teams.
