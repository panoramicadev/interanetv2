...
- **Promesas de Compra Semanales**: Weekly purchase promise tracking system allowing salespeople to register client purchase commitments per week with amount. Features include week-based promise registration (YYYY-WW format), compliance comparison matching promises vs actual sales (facturas + NVV) for committed weeks, visual compliance indicators (green=exceeded, blue=met, red=not met), client search functionality, weekly navigation controls, and complete CRUD operations. Accessible to admin, supervisor, and salesperson roles via sidebar with HandHeart icon. Backend compares promises against fact_ventas and nvv_pending_sales tables for the committed week period.
- **ETL Data Warehouse Tables**: Database schema for ETL processes in the PostgreSQL schema "ventas" with 8 staging tables (stg_maeedo, stg_maeddo, stg_maeen, stg_maepr, stg_maeven, stg_tabbo, stg_tabpp) for importing data from SQL Server and a fact_ventas table with 81 columns (79 original + ESDO + ESPGDO) for the final denormalized data. Tables include optimized indexes for query performance (ix_fact_ventas_feemli_id, ix_fact_ventas_cliente, ix_fact_ventas_producto, ix_fact_ventas_vendedor, ix_fact_ventas_bodega). ETL execution log (watermark) table tracks all ETL runs with etl_name identifier for multi-ETL support, execution status, period, document types, branches processed, records count, execution time, detailed statistics in JSON format, and watermark timestamp for last processed date. **Two ETL modes available**: (1) Full Annual ETL (server/etl-process.ts) for manual complete data loads, processes entire years sequentially; (2) **Automatic Incremental ETL (server/etl-incremental.ts)** runs every 15 minutes via scheduler, uses watermark-based incremental loading to process only new/modified records since last execution, filters data from 2022 onwards, performs UPSERT operations (delete old + insert updated), includes API endpoints POST /api/etl/execute?etlName={name} (manual trigger, admin/supervisor only) and GET /api/etl/status?etlName={name} (monitoring and history). Both modes extract from SQL Server (PANORAMICA database), load to staging tables, process into fact_ventas with calculated monto column (CAPRCO2 * PPPRNE for regular docs, CAPRCO2 * PPPRNE * -1 for NCV credit notes), support multiple branches (004, 006, 007), process document types (FCV, GDV, FVL, NCV, BLV, FDV), include descriptive names (nokoen, nokofudo, nokoprct, nosudo) via LEFT JOINs to MAEEN, TABFU, TABSU, TABBO tables, handle empty/null numeric values with cleanNumeric helper, and filter GDV to only load pending documents. Automatic scheduler initializes on app startup (runs first ETL after 30 seconds, then every 15 minutes). **ETL Monitor Panel**: Visual admin interface at /etl-monitor route with multi-ETL support via tab-based interface. Features include execution status display (success/error/running), manual ETL trigger button (admin/supervisor only), last execution timestamp and statistics, execution history with detailed logs, watermark tracking, and auto-refresh every 10 seconds. Each ETL process identified by unique etlName (defaults to 'ventas_incremental'), allowing future expansion with additional ETL processes (e.g., 'compras_incremental'). UI configured via ETL_CONFIGS array with id, name, description, and icon for each process. Backend functions accept etlName parameter for filtering logs and watermark data. Accessible via sidebar for admin and supervisor roles with Database icon. 100% schema compatibility verified against ERP CSV exports with all 81 columns matched. Production deployment files available: production-etl-setup.sql (schema creation script) and INSTRUCCIONES-ETL-PRODUCCION.md (deployment guide).
