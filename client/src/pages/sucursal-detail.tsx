...
export default function SucursalDetail({
  ...
  const isComparativeMode = useMemo(() => {
    console.log("🔍 [branch-detail] Detectando modo comparativo:", {
      period: selection.period,
  ...
  const { data: clients = [], isLoading: isLoadingClients } = useQuery<BranchClient[]>({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`/api/sales/branch/${branchName}/clients?${params}`, { credentials: 'include' });
  ...
  const { data: salespeople = [], isLoading: isLoadingSalespeople } = useQuery<BranchSalesperson[]>({
    queryKey: ['/api/sales/branch', branchName, 'salespeople', selectedPeriod, filterType],
    queryFn: async () => {
      const params = new URLSearchParams();
      params.append('period', selectedPeriod);
      params.append('filterType', filterType);
      const res = await fetch(`/api/sales/branch/${branchName}/salespeople?${params}`, { credentials: 'include' });
      if (!res.ok) throw new Error(`${res.status}: ${res.statusText}`);
      return await res.json();
    },
    enabled: !!branchName,
  ...
  // Fetch branch goal (only for monthly periods)
  const { data: goalData } = useQuery({
    queryKey: ['/api/goals/progress', selectedPeriod, branchName],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (selectedPeriod) {
    ...
    enabled: !!branchName && filterType === 'month', // Only fetch for monthly view
  });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-red-600">Sucursal no encontrado</h1>
          {onBack && (
  ...
  const totalSalesFromClients = clients.reduce((sum: number, client: BranchClient) => sum + client.totalSales, 0);
    ...
    <div className="min-h-screen">
      <div className="w-full">
        ...
        <main className="p-3 sm:p-4 lg:p-6 space-y-4 lg:space-y-6">
          ...
          {(() => {
            console.log("🎬 [branch-detail] RENDER:", {
              isComparativeMode,
