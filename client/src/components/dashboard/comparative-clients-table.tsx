...
interface ComparativeClientsTableProps {
  periods: Array<{ period: string; label: string; filterType: "day" | "month" | "year" }>;
}

export default function ComparativeClientsTable({ periods }: ComparativeClientsTableProps) {
  // Fetch clients data for all periods using useQueries to respect Rules of Hooks
  const clientsQueries = useQueries({
    queries: periods.map(({ period, filterType }) => ({
      queryKey: [`/api/sales/top-clients?limit=5000&period=${period}&filterType=${filterType}`],
      queryFn: async () => {
        const res = await fetch(
          `/api/sales/top-clients?limit=5000&period=${period}&filterType=${filterType}`, 
          { credentials: "include" }
        );
        if (!res.ok) throw new Error('Failed to fetch');
        return await res.json() as TopClientsResponse;
      }
    }))
  });

  const isLoading = clientsQueries.some(q => q.isLoading);
  const allData = clientsQueries.map(q => q.data?.items || []);
    ...
    <div className="bg-white border rounded-lg p-6">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {allClients.slice(0, 20).map((clientName) => {
              return (
