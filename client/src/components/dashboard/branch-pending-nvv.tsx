...
interface BranchPendingNVVProps {
  branch: string;
}
...
export default function BranchPendingNVV({
  branch
}: BranchPendingNVVProps) {
  const [showAll, setShowAll] = useState(false);
  const [isCollapsed, setIsCollapsed] = useState(true);
  
  const { data: nvvData, isLoading } = useQuery<NVVRecord[]>({
    queryKey: [`/api/nvv/by-branch`, branch],
    queryFn: async () => {
      const params = new URLSearchParams({
        branch
      });
      const response = await fetch(`/api/nvv/by-branch?${params}`, {
        credentials: 'include'
      });
      if (!response.ok) {
  ...
  const groupedByClient: ClientGroup[] = nvvData ? 
    Object.values(
      nvvData.reduce((acc, record) => {
        ...
        acc[uniqueKey].totalAmount += record.VABRDO;
  ...
  const totalPendingAmount = nvvData?.reduce((sum, record) => sum + record.VABRDO, 0) || 0;
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Notas de Venta Pendientes - {branch}
          </CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              No hay notas de venta pendientes para la sucursal {branch}
            </p>
          </div>
        </CardContent>
      </Card>
              ...
              <div>
                <Accordion type="single" collapsible className="space-y-2">
                  ...
                  <AccordionItem 
                    ...
                    data-testid={`branch-client-group-${clientGroup.uniqueKey}`}
                  >
                    <AccordionTrigger 
                      className="px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800 hover:no-underline"
                      data-testid={`branch-client-trigger-${clientGroup.uniqueKey}`}
                    >
                          ...
                          <Table>
                            ...
                            <TableBody>
                              {clientGroup.records.map((record) => (
                                <TableRow key={record.id} data-testid={`branch-nvv-detail-${record.id}`}>
                                  ...
                                  <TableCell>
                                    <div className="max-w-xs">
                                      ...
                                      <div className="text-xs text-gray-500">{record.ENDO}</div>
                                    </div>
                                  </TableCell>
                                  <TableCell>
                ...
                {groupedByClient.length > 5 && (
                  <div className="mt-3 text-center">
                    <Button
                      ...
                      data-testid="button-toggle-branch-nvv"
                    >
                      {showAll ? (
                        <>
