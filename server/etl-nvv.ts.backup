  ...
  try {
    ...
    for (const row of maeddo.recordset) {
      ...
      if (existingRecord) {
        ...
        if (!oldStatus && newStatus === 'C') {
          status_changes++;
          logger.info(`Cambio de estado detectado: NVV cerrada`, {
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.execute(sql`
        ...
        LEFT JOIN nvv.stg_maeven_nvv fu ON dd.kofulido = fu.kofu
        LEFT JOIN nvv.stg_tabru_nvv ru ON en.ruen = ru.koru
        LEFT JOIN nvv.stg_tabbo_nvv bo ON ed.suli = bo.suli AND ed.bosulido = bo.bosuli
      `);
    });
